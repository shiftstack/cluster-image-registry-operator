...
func (d *driver) ConfigEnv() (envs envvar.List, err error) {
	...
	tenantDomain := replaceEmpty(d.Config.TenantDomain, cfg.TenantDomain)
	tenantDomainID := replaceEmpty(d.Config.TenantDomainID, cfg.TenantDomainID)
	...
	if tenantDomain != "" {
		envs = append(envs, envvar.EnvVar{Name: "REGISTRY_STORAGE_SWIFT_TENANTDOMAIN", Value: tenantDomain})
	}
	if tenantDomainID != "" {
		envs = append(envs, envvar.EnvVar{Name: "REGISTRY_STORAGE_SWIFT_TENANTDOMAINID", Value: tenantDomainID})
	}
	if regionName != "" {
